...
def process_site(client, data_set_id, lookback_minutes, site):
    assets = client.assets.list(asset_subtree_external_ids=site.lower().replace(" ", "_"), limit=None)
    timeseries = client.time_series.list(asset_subtree_external_ids=site.lower().replace(" ", "_"), limit=None)
